...
class HybridRetriever(BaseRetriever):
    ...
    def _get_relevant_documents(
        ...
        *,
        ...
        for i, q in enumerate(query_variations, 1):
            ...
            # current value of `q` instead of the loop variable by reference.
        ...
        if self.enable_rerank:
            fused_docs = retry_call(lambda: self._rerank(query, fused_docs))
    ...
    def _accumulate_rrf_scores(
        ...
        def _add(docs: List[Document]) -> None:
            for rank, doc in enumerate(docs):
                # After normalisation, doc_id is always present in metadata
    ...
    def _create_multiple_reformulated_queries(self, query: str) -> List[str]:
        ...
        # BUG FIX: the prompt was constructed but `query` (not `prompt`) was
        # passed to .invoke(), so the LLM never saw the reformulation instructions.
        prompt = (
