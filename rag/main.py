...
if __name__ == "__main__":
    try:
        ...
        delete_all_documeents_before_ingest = config.get("delete_all_documeents_before_ingest", False)  # default to False if not specified
        ...
        if docs:
            ...
            if delete_all_documeents_before_ingest
            ...
            keyword_store_manager = KeywordStoreManager(index_name=bm25_index_name)
             if delete_all_documeents_before_ingest:
                keyword_store_manager.delete_index()  # Clear existing index before ingesting new documents
